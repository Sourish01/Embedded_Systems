...
extern tCANMsgObject rxMessage;
extern uint8_t rxData[8];
extern void CAN0_Handler(void);

void CAN_Config(void) {
    // Enable CAN0 and Port E peripherals
    ...
    // Configure GPIO pins for CAN on Port E
    GPIOUnlockPin(GPIO_PORTE_BASE, GPIO_PIN_4 | GPIO_PIN_5);
    GPIOPinConfigure(GPIO_PE4_CAN0RX);  // PE4 as CAN0RX
    GPIOPinConfigure(GPIO_PE5_CAN0TX);  // PE5 as CAN0TX
    GPIOPinTypeCAN(GPIO_PORTE_BASE, GPIO_PIN_4 | GPIO_PIN_5);

    // Initialize CAN controller
    CANInit(CAN0_BASE);

    // Configure CAN with the 50 MHz system clock and 500 kbps bit rate
...
//    IntPrioritySet(INT_CAN0, 0x40); // Priority 2 (higher)
    ...
    rxMessage.ui32MsgLen = sizeof(rxData);
